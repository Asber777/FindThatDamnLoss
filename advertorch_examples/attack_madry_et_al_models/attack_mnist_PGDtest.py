...
test_iter = [50,100,200,500,1000]
for iter in test_iter:
    adversary = LinfPGDAttack(
        model, loss_fn=nn.CrossEntropyLoss(reduction="sum"), eps=0.3,
        nb_iter=iter, eps_iter=0.01, rand_init=False, clip_min=0.0, clip_max=1.0,
        targeted=False)

    label, pred, advpred, _ = multiple_mini_batch_attack(
        adversary, loader, device="cuda")

    print("When nb_iter is {}, Accuracy: {:.2f}%, Robust Accuracy: {:.2f}%".format(
        iter,
